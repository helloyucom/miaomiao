<template>
    <div class="search_body">
        ...
        <div class="search_result">
            <h3>电影/电视剧/综艺</h3>
            <ul>
                <li  v-for="(item, index) of moviesList" :key="index">
        ...
        methods: {
            cancelRequest(){
                if(typeof this.source ==='function'){
                    this.source('axios终止多次请求')
                }
            }
        },
        watch: {
            message: function(newVal) {     // 监听message
                // console.log(newVal);
                this.cancelRequest();   // 请求数据之前执行终止请求的方法
                var that = this;
